    ...
    public partial class frmSetting : Form
    {
        ...
        SettingConfig GetConfigFromSetting()
        {
            try
            {
                SettingConfig config = new SettingConfig()
                {
                    ...
                    fileMgr_bMaxFolder = checkBox8.Checked,
                    fileMgr_dwMaxFolder = (int)numericUpDown3.Value,
                    fileMgr_bMaxFile = checkBox9.Checked,
        ...
        void Save()
        {
            try
            {
                ...
                //FileMgr
                if (!Write("FileMgr", "bMaxFolder", config.fileMgr_bMaxFolder))
                    throw new Exception("Write bMaxFolder error.");

                if (!Write("FileMgr", "bMaxFile", config.fileMgr_bMaxFile))
                    throw new Exception("Write bMaxFile error.");
