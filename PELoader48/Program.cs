...
namespace PELoader48
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            bool process64 = IntPtr.Size == 8;
            x64PELoader pe = new x64PELoader(File.ReadAllBytes(args[0]));
            bool pe32 = pe.Is32BitHeader;

            if (process64 && pe32)
            {
                throw new Exception("x64 process cannot execute x86 PE");
            }

            if (!process64 && !pe32)
            {
                throw new Exception("x86 process cannot execute x64 PE");
            }

            if (process64)
            {
                // x64 PE loader
                RunX64Loader(File.ReadAllBytes(args[0]));
            }
            else
            {
                // x86 PE loader
                RunX86Loader(File.ReadAllBytes(args[0]));
            }

            
        }

        static void RunX86Loader(byte[] abPE)
        {
            PELoader pe = new PELoader(abPE);
        ...
        static void RunX64Loader(byte[] abPE)
        {
            x64PELoader pe = new x64PELoader(abPE);
